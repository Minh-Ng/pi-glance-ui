...
test("private layout patches require explicit version-scoped consent", async (t) => {
  ...
  writeFileSync(configPath, JSON.stringify({ patchesVersion: "0.80.5" }));
  ...
  assert.deepEqual(target.notifications.at(-1), {
    message: "Glance UI: private patches remain off on Pi 0.80.6; approval was for Pi 0.80.5.",
    level: "warning",
  });
  ...
  assert.equal(JSON.parse(readFileSync(configPath, "utf8")).patchesVersion, "0.80.5");

  await command("install-patch", target.ctx);
  assert.equal(target.confirmationRequests.length, 2);
  assert.match(target.confirmationRequests[1].message, /in-memory prototype patches to Pi 0\.80\.6/);
  ...
  assert.deepEqual(target.notifications.at(-1), {
    message: "Glance UI private patches: on for Pi 0.80.6 · saved",
    level: "info",
  });
